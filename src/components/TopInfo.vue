 <template>
  <div class="top-info mrb10 ">
    <div class="flex box-infomation" style="margin-bottom: 5px;" v-if="info">
      <div class="icon-pin disable_ccp_page_dex" @click="handleChange()">
        <BaseIcon :url="'pin2.png'" class="cursor"/>
      </div>
      <div class="big-box-info">
        ...
        <div class="box2 disable_ccp_page_dex">
          <div class="fw600 fs18 color-blue">{{__t('Thông tin khám gần nhất')}}</div>
          <TopInfomationDetail :info="info" :Info2="Info2" :VisitType="_VisitType" :VisitId="_VisitId"/>
        </div>
      </div>
    </div>
  </div>
...
export default {
  name: 'top-info',
  props: ['info', 'VisitType', 'hideCode', 'VisitId', 'Info2'],
  data () {
    return {
      ALLERGY: ALLERGY
    }
