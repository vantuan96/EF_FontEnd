...
// import SignificantMedicationsFakeData from '@/common/SignificantMedications.js'
export default {
  /**
  ...
  methods: {
    loadData () {
      new DischargeInformation({hidemsg: true, notRedirect: true}).create({Id: this._VisitId}, 'SyncReadOnlySignificantMedications').then(response => {
        this.datas = response.length ? response : null
      })
    },
    collapse () {
      this.open = !this.open
