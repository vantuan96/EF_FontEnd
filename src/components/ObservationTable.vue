  ...
  <table class="table table-bordered table-bordered-focus" id="disable-x388ahxh">
    <thead>
      ...
      <template v-if="onEtr">
        <tbody v-if="data.Datas && data.Datas.length">
          <template v-for="(item, index) in data.Datas" :class="{'is_new': !item.Id}">
            <tr :key="index + 'x'">
              <template v-if="!readonly && (!item.Id || (item.Id && item.Fullname === $store.state.account.Username))">
      ...
      <tfoot>
        ...
        <!-- <tr v-if="data.Datas && onEtr && data.Datas.filter(item => !item.Id).length">
          <td colspan="10">
            <div class="row">
              <div class="col-md-2"><button type="button" class="btn btn-block" @click="close">{{__t('Hủy')}}</button></div>
              <div class="col-md-3"><button type="button" class="btn btn-block v-yellow-btn" @click="submit">{{__t('Lưu')}}</button></div>
            </div>
          </td>
        </tr> -->
      </tfoot>
...
export default {
  ...
  props: ['VisitId', 'onEtr', 'Type', 'readonly'],
  components: {
    VDatePicker
  },
  data () {
  ...
  methods: {
    ...
    submit () {
      ...
      new ObservationChart({VisitType: this.Type}).update(this.VisitId, this.data).then(response => {
        this.toastedSuccess()
        this.loadData()
      })
    }
