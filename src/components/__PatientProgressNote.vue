<template>
  <div class="ed-info box-collapse open">
    <div class="ed-customer-title">
      ...
      <button class="btn btn-xs btn-flat btn-primary pull-right" @click="openForm()">{{__t('btn.nhap_du_lieu')}}</button>
    </div>
        ...
        <template v-if="Datas && Datas.length">
          <thead>
            <tr>
              ...
              <th width="1" class="no-wrap">Người tạo</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div class="box v-model-content-popup">
        ...
        <div class="box-footer padding-15">
          <div class="row">
            ...
            <div class="col-md-6"><button :disabled="!item.Note || !item.Interventions || !item.NoteAt" type="button" class="btn btn-block v-yellow-btn" @click="submit">{{__t('btn.luu')}}</button></div>
          </div>
        </div>
      </div>
    </modal>
...
import PatientProgressNotes from '@/services/ED/PatientProgressNotes'
...
export default {
  ...
  methods: {
    submit () {
      ...
      new PatientProgressNotes().update(this.EdId, data).then(response => {
        this.toastedSuccess()
        this.close()
        this.getData()
      })
    ...
    getData () {
      new PatientProgressNotes().find(this.EdId).then(response => {
