<template>
  <span class="labval">Mạch <i class="unit-label">(lần/phút)</i>: <!----> <!----></span>
  <!-- <div class="row">
    <div class="col-md-12" v-if="value">
      <div class="form-group">
        ...
        <div :value="item" :key="index + '-' + item.Code"  v-for="(item, index) in value.Items">
          ...
          <div v-if="groupDefaultLib">
            <list-select
              @select="selectedValue"
              :name="item.Code"
              :data="item.Data"
              :defaultValue="item.Value"
              :title="__t('Tham khảo thông tin khám bình thường')"
              :groupDefaultLib="groupDefaultLib"
            />
          </div>
          <div v-if="readonly">
            <p class="fake-input disable" v-if="!item.IsReadOnly">{{item.Value}}</p>
          </div>
...
<script>
export default {
  name: 'MdInputTextInline',
  props: ['value', 'hidelabel', 'defaultLib', 'groupDefaultLib', 'placeholder', 'readonly'],
  mixins: [],
  watch: {
    value: function (val) {
