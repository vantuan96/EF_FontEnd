    ...
    <div class="a4-page box" :id="printId || 'printMe'">
      ...
      <div>
        ...
        <div style="padding-left: 5px;">
          <p style="margin: 7px 0;font-size: 14px;line-height: 1.5;" :data="item" :key="index" v-for="(item ,index) in data.Assessment">+ <span>{{item.ViName}}/<i> {{item.EnName}}: </i>{{item.Value}}</span></p>
        </div>
      </div>
...
export default {
   ...
   */
  name: 'KLMView',
  props: ['data', 'isTranView', 'printId', 'viewOnly'],
  mixins: [],
  data () {
    return {
      MEDICALREPORT_PHY: '',
