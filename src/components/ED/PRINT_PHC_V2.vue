      ...
      <article>
        <table>
          <tr>
            ...
            <td align="center">
              <h2>BẢNG KIỂM CHUẨN BỊ VÀ BÀN GIAO NGƯỜI BỆNH TRƯỚC PHẪU THUẬT</h2>
              <h3><i>PRE-OPERATIVE VERIFICATION AND HANDOVER CHECKLIST</i></h3>
            </td>
        ...
        <table class="tableboder" :class="bindCalss()">
          <tr>
                ...
                <tr>
                  <template :data="item"  v-for="(item,index) in MASTERDATA['PHCTOP'].Items" >
                    <td :key="index" v-if="item.DataType === 'RadioBox'" class="col-30">
                      ...
                      <template v-if="index === 2" style="no-wrap">Phẫu thuật trong ngày/ Day Surgery</template>
                      <template v-else><vlabel :data="item" /></template>
                    </td>
                  </template>
                </tr>
              </table>
            ...
            <td rowspan="4" v-if="MASTERDATA['PHC68']">
              <p><vlabel :data="MASTERDATA['PHC68']" /></p>
              <p class="text-center font18">
                <b>
                  <template v-for="item in MASTERDATA['PHC68'].Items">
                    {{item.Value}}
                  </template>
                </b>
              </p>
        ...
        <table class="tableboder" :class="bindCalss()">
          <tr>
            ...
            <th rowspan="2" class="col-1">Phòng mổ/OR</th>
          </tr>
          <tr>
          ...
          <tr v-if="MASTERDATA['PHC36']">
            <td>5. <vlabel :data="MASTERDATA['PHC36']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCPMR']">
            <td>6. Đã hoàn thành biên bản hội chẩn trước mổ, có trong hồ sơ bệnh án/ Preoperative Joint Consultation form completed as required & present</td>
          ...
          <tr v-if="MASTERDATA['PHCSCP']">
            <td>7. <vlabel :data="MASTERDATA['PHCSCP']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCLRC']">
            <td>8. <vlabel :data="MASTERDATA['PHCLRC']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCXUS']">
            <td>
              <div style="display: flex; align-items: center; flex-wrap: wrap;">
                <p style="margin: 0 15px 0 0;">9. Phim {{PHCXUSANS('vi')}} đã được kiểm tra và kết quả có trong hồ sơ bệnh án /{{PHCXUSANS('en')}} checked & available</p>
                <p style="margin: 0;">{{MASTERDATA['PHC41'].ViName}}/ <i>{{MASTERDATA['PHC41'].EnName}}</i>: {{MASTERDATA['PHC41'].Items[0].Value}}</p>
              </div>
            </td>
          ...
          <tr v-if="MASTERDATA['PHCFF0']">
            <td>10. <vlabel :data="MASTERDATA['PHCFF0']" /> <i>{{MASTERDATA['PHCFF0'].Items[0].Value}}</i></td>
          ...
          <tr v-if="MASTERDATA['PHCEG0']">
            <td>11. <vlabel :data="MASTERDATA['PHCEG0']" /> <i>{{MASTERDATA['PHCEG0'].Items[0].Value}}</i></td>
          ...
          <tr v-if="MASTERDATA['PHCAKA']">
            <td>12. <vlabel :data="MASTERDATA['PHCAKA']" /> <i>{{MASTERDATA['PHCAKA'].Items[0].Value}}</i></td>
          ...
          <tr v-if="MASTERDATA['PHC43']">
            <td>13. <vlabel :data="MASTERDATA['PHC43']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCSIC']">
            <td>14. <vlabel :data="MASTERDATA['PHCSIC']" /></td>
          ...
          <tr v-if="MASTERDATA['PHC48']">
            <td>15.1. <vlabel :data="MASTERDATA['PHC48']" /></td>
          ...
          <tr v-if="MASTERDATA['PHC53']">
            <td>15.2. <vlabel :data="MASTERDATA['PHC53']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCAPO']">
            <td>16. <vlabel :data="MASTERDATA['PHCAPO']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCNCA']">
            <td>17. <vlabel :data="MASTERDATA['PHCNCA']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCAMR']">
            <td>18. <vlabel :data="MASTERDATA['PHCAMR']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCJBP']">
            <td>19. <vlabel :data="MASTERDATA['PHCJBP']" /></td>
          ...
          <tr v-if="MASTERDATA['PHC58']">
            <td>20. <vlabel :data="MASTERDATA['PHC58']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCACC']">
            <td>21. <vlabel :data="MASTERDATA['PHCACC']" /><i> {{MASTERDATA['PHCACC'].Items[0].Value}}</i></td>
          ...
          <tr v-if="MASTERDATA['PHC63']">
            <td>22. <vlabel :data="MASTERDATA['PHC63']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCICI']">
            <td>23. <vlabel :data="MASTERDATA['PHCICI']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCCBBS']">
            <td>24. <vlabel :data="MASTERDATA['PHCCBBS']" /></td>
          ...
          <tr v-if="MASTERDATA['PHCOTH']">
            <td>25. <vlabel :data="MASTERDATA['PHCOTH']" /><i> {{MASTERDATA['PHCOTH'].Items[0].Value}}</i></td>
        ...
        <table :class="bindCalss()" style="margin-top: 10px;">
          <tr>
            <td class="col-33 text-center">
              ...
              <p>{{PHCCONFIRM_WDN ? PHCCONFIRM_WDN.Fullname : ''}}</p>
            </td>
            <td class="col-33 text-center">
              ...
              <p>{{PHCCONFIRM_ETN ? PHCCONFIRM_ETN.Fullname : ''}}</p>
            </td>
            <td class="col-33 text-center">
              ...
              <p>{{PHCCONFIRM_RGN ? PHCCONFIRM_RGN.Fullname : ''}}</p>
            </td>
          </tr>
        </table>
        <p style="font-size: 12px">A03_035_201022_VE</p>
      </article>
    </section>
...
export default {
  ...
  watch: {
    PHCCONFIRM_WDN: {
      handler (val) {
        console.log('PHCCONFIRM_WDN', val)
      }
    }
  },
  mounted () {
  ...
  methods: {
    getConfirm () {
      new EIOService().find('ConfirmForms/GetList/' + this.PreOperativeProcedureHandoverChecklistsData.Id).then(async res => {
        this.listUserConfirm = await res
        this.listUserConfirm.map(item => {
          if (item.ConfirmType === 'PHCCONFIRM_WDN') {
            this.PHCCONFIRM_WDN = item
          }
          if (item.ConfirmType === 'PHCCONFIRM_ETN') {
            this.PHCCONFIRM_ETN = item
          }
          if (item.ConfirmType === 'PHCCONFIRM_RGN') {
            this.PHCCONFIRM_RGN = item
          }
        })
      }).catch((e) => {
        console.log(e)
