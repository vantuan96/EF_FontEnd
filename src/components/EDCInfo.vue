      ...
      <div class="box-body">
        <div class="row">
          <div class="col-md-12">
            ...
            <p class="cursor" v-if="EOCInfo.AcceptBy" @click="open">{{__t('Bệnh nhân')}} <b>{{__label(EOCInfo.Status)}}</b> {{__t('tại khu vực nội soi')}}, {{__t('Tiếp nhận bởi')}}: <AdInfo :ad="EOCInfo.AcceptBy" />
              <br/>
              <!-- <i v-if="!readonly" class="link-text">{{__t('Xem chi tiết')}}</i> -->
            </p>
            ...
            <div v-if="EOCInfo.AcceptBy">
              <!-- {{EOCInfo.VisitId}} -->
              <DetailInfo :visitId="EOCInfo.VisitId" :isEOC="true" :readonly="readonly"/>
              <!-- <modal name="histoyr-modalxx" transition="pop-out" height="auto" :scrollable="false" :width="modalWidth">
                <div class="box v-model-content-popup">
                  <div class="box-body padding-15" v-if="hasPopup">
                    <EOCProcedureSafetyChecklist :VisitId="EOCInfo.VisitId" />
                  </div>
...
<script>
import $ from 'jquery'
import EOCProcedureSafetyChecklist from '@/pages/EOC/View'
...
export default {
  ...
  components: {
    EOCProcedureSafetyChecklist, DetailInfo
  },
  mixins: [],
  data () {
    return {
  ...
  mounted () {
    ...
    console.log('EOC show 1:', this.value)
    console.log('EOC show 2:', this.EOCInfo)
  },
  methods: {
    close () {
      this.hasPopup = false
