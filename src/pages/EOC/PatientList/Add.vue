<template>
  <div class="main-content" id="add-customer-page">
    ...
    <!-- <p class="text-center"><button type="button" class="btn" @click="submitCustomersEOC()">{{__t('customer.btn_tao_moi')}}</button></p> -->
  </div>
</template>
<script>
/* ============
...
export default {
  ...
  methods: {
    ...
    choseAndgo2InitialAssessment () {
      ...
      if (vsCode) {
        if (this.TransferData) {
          ...
          this.submitCustomersEOC(vsCode ? vsCode.join(',') : '')
        }
      }
    },
    submitCustomersEOC (VisitCode) {
      this.customer.VisitCode = VisitCode
      new CustomersEOC({hidemsg: true}).create(this.customer).then(response => {
    ...
    SyncVisitCodeByPid () {
      new CustomersEOC({checkTranfer: 'whatever', hidemsg: true}).find('SyncVisitCodeByPID/' + this.TransferData.PID).then(response => {
    ...
    SyncVisitCode (customer) {
      ...
      new CustomersEOC({checkTranfer: 'whatever', hidemsg: true}).find('SyncVisitCodeByPID/' + customer.PID).then(response => {
        this.$modal.show('visitcode-modal')
        this.visitcodes = response
      }).catch(e => {
        if (e.data && e.data.TransferExit) {
    ...
    create (VisitCode) {
      new EocService().update('/customer-form-other-specialty/' + this.TransferData.Id, {VisitCode}).then(response => {
        this.$router.push({name: 'EOCInitialAssessmentShort', params: {Id: response.Id}})
      })
    },
    updateVisitCode (visitcode) {
      this.customer.VisitCode = visitcode
      new CustomersEOC().create(this.customer, 'ManualUpdate/' + this.opdid).then(response => {
        this.toastedSuccess()
        // this.getData()
    ...
    submit () {
      ...
      new Customer(this.filters)
        ...
        .then(response => {
          ...
          if (this.ImSuperAdmin()) this.submitCustomersEOC('TN.X.Y.X.' + new Date().getTime())
        })
    }
  }
}
