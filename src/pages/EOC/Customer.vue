<template>
  <div class="main-content" id="add-customer-page">
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <FakeInput style="margin: 0 !important" type="text" class="form-control" v-model="customer.WorkPlace" :placeholder="__t('customer.quoc_tich')" />
                </div>
              </div>
            </div>
            <div class="row">
    ...
    <modal :name="this.$route.params.Id + '-modal'" transition="pop-out" height="auto" :scrollable="true" :width="modalWidth">
...
export default {
  ...
  methods: {
    getData () {
      new CustomersEOC().find(this.$route.params.Id).then(response => {
    ...
    submit () {
      ...
      new CustomersEOC().create(this.customer, '/ManualUpdate/' + this.$route.params.Id).then(response => {
        this.checkBoder = 0
        this.toastedSuccess()
        this.getData()
    ...
    ShowSyncVisitCode () {
      new CustomersEOC().find('SyncVisitCode/' + this.$route.params.Id).then(response => {
    ...
    openTable () {
      this.$modal.show(this.$route.params.Id + '-modal')
    },
    updateEd (data) {
      this.customer = data
      ...
      this.$modal.hide(this.$route.params.Id + '-modal')
      this.submit()
    },
