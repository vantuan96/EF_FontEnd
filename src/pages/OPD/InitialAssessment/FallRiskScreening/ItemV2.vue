<template>
  <div v-if="loaded">
    <div v-if="allowEdit && !ViewOnly">
      <div class="form-control" v-if="false"><button class="btn btn-sync-data pull-right" @click="syncData()">{{__t('Lấy dữ liệu đánh giá gần nhất trong vòng 24 giờ')}}</button></div>
      ...
      <div v-if="DataSubmit.Version >= 9">
        <Confirm
          ...
          :FormCode="'OPD_A02_007_220321_VE'"
          :ReadOnly="!allowEdit"
      ...
      <logs :EdId="this._VisitId" :Type="'OPD/InitialAssessments/FallRiskScreening'" />
    </div>
  ...
  computed: {
    ...
    allowEdit () {
      if (this.DataSubmit.Version >= 9) {
        return (this.DataSubmit.CreatedBy === this.getUser() || this.ImSuperAdmin()) && !this.DataSubmit.IsLocked && this.hasRole('OINAS8') && !(this.ConfirmInfo.length > 0 && this.ConfirmInfo[0].ConfirmBy)
      }
      return (this.DataSubmit.CreatedBy === this.$store.state.account.Username || this.ImSuperAdmin()) && !this.DataSubmit.IsLocked && this.hasRole('OINAS8')
      // return true
    }
  },
  methods: {
    ...
    removeETRConfirmed () {
      new InitialAssessment().update('Delete/' + this._VisitId, this.DataSubmit).then(response => {
        this.go2Home()
      })
    },
    syncData () {
    ...
    getData () {
      new InitialAssessment().find('FallRiskScreening/' + this.FormCode + '/' + this._VisitId + '/' + this._ItemId).then(response => {
        ...
        if (!this.MASTERDATA['OPDFRSFOPLOC'].Items[0].Value) {
          let cilinicData = this.Clinics.find(e => e.ViName === response.ClinicOfDoctorPrimary)
          this.MASTERDATA['OPDFRSFOPLOC'].Items[0].Value = cilinicData.value
        }
        if (!this.MASTERDATA['OPDFRSFOPUTH'].Items[0].Value) {
