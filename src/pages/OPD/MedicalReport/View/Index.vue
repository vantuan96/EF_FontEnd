  ...
  <div class="a4-page box has-translation" :id="printId || 'printMe'" v-if="tmp">
    ...
    <div v-else>
      ...
      <!-- <div style="display: flex;">
        <span>/</span>
        <p>{{formatText(dataTrans.Translation.ClinicalExaminationAndFindings)}}</p>
      </div> -->
    </div>
    <div>
    ...
    <div>
      <p @dblclick="addBreakPage" class="hasBreakPage" style="margin: 7px 0;font-size: 14px;line-height: 1.5;"><b>Ngày hẹn tái khám</b>/ <i>Date of next appointment: </i>{{(dataParent.DateOfNextAppointment ? dataParent.DateOfNextAppointment : data.DateOfNextAppointment)}}</p>
    </div>
    <div v-if="checkHide()">
      <p @dblclick="addBreakPage" class="hasBreakPage" style="margin: 7px 0;font-size: 14px;line-height: 1.5;"><b>Ngày hẹn trả KQ</b>/ <i>The return date results: </i>{{(dataParent.AppointmentDateResult ? dataParent.AppointmentDateResult : data.AppointmentDateResult) | formatDateWithoutSecon}}</p>
    </div>
    <div>
    </div>
    <table width="100%">
    ...
    <div v-if="isTranView" class="print-area only-web-page">
    ...
    <div v-if="dataParent.Version" class="only-web-page">
      <Confirm
        class="mt-10"
        style="display: none;"
...
export default {
  ...
  props: ['data', 'isTranView', 'printId', 'site', 'Status', 'PkntVersion', 'dataParent', 'dataTrans', 'VisitType'],
  data () {
    return {
      tmp: null,
      MEDICALREPORT_PHY: '',
  ...
  mounted () {
    console.log(this.dataTrans, 'check tmp what data have')
    ...
    if (this.isJson(this.dataTrans.ClinicalExaminationAndFindings)) {
      this.tmp.ClinicalExaminationAndFindings = this.JSONParse(this.tmp.ClinicalExaminationAndFindings) || []
    }
  ...
  methods: {
    ...
    formatText (text) {
      if (text) {
        let handleText = text.toString()
        return handleText.replaceAll('/n', '\n')
        // return handleText.split(handleText.lastIndexOf('\n'))
      }
      return text
    }
  }
