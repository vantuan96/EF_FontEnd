  ...
  <div class="main-content" id="Home-page">
    <h1 class="title-page">{{ __t('home.danh_sach_benh_nhan') }}</h1><div class="row v-flexbox">
        ...
        <div class="col-md-2 col-sm-2 no-padding-left">
          <a style="margin-bottom: 0px;" type="button" href="/#/opd/add" class="btn v-yellow-btn pull-right btn-home-add">
            <span>
    ...
    <div class="v-tab">
      <ul class="nav nav-tabs" role="tablist">
        <li role="presentation" :class="activeClass('ListPatientInOPD')">
          <router-link :to="{name: 'ListPatientInOPD'}">
            {{ __t('Danh sách người bệnh') }}
          </router-link>
        </li>
        <li role="presentation" :class="activeClass('ListPatientTransferOPD')" v-if="$store.state.account.Position.includes('Nurse') || $store.state.account.Position.includes('Administrator')">
          <router-link :to="{name: 'ListPatientTransferOPD'}">
