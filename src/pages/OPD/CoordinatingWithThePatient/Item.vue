   ...
   <div v-else class="content mt-20">
    ...
    <div class="box form-content mbt30">
     ...
     <div class="flex" v-if="MASTERDATA['IPDIAACOORDINB'].Items[0].Value">
        ...
        <div class="ml-5">{{Customer.Fullname}}</div>
     </div>
     <div v-if="MASTERDATA['IPDIAACOORDIGDNB'].Items[0].Value">
    ...
    <div class="box form-content mbt30">
     ...
     <p class="ml-5">
      <b>- {{ __t("Họ tên người bệnh") }}:</b
      ><span class="tab404">{{ Customer.Fullname || "" }}</span>
      <b>{{ __t("Tuổi") }}:</b> <span class="tab404">{{ Customer.AgeFormated | formatAge }}</span>
      <b>{{ __t("Giới tính") }}:</b>
      <span class="tab404">{{ __t(GENDER[Customer.Gender]) }}</span>
     </p>
     <p>
      <b>- {{ __t("Dân tộc") }}:</b
      ><span class="tab404">{{ Customer.MOHEthnic }}</span
      >
      <p>
      <b>- {{ __t("Nghề nghiệp") }}:</b
      ><span class="tab404">{{ Customer.MOHJob || "N/A" }}</span
     ...
     <p>
      <b>- {{ __t("Địa chỉ") }}:</b
      ><span class="tab404">{{ Customer.Address }}</span>
     </p>
     <p>
      <b>- PID:</b
      ><span class="tab404">{{ Customer.PID }}</span>
     </p>
     <p>
      <b>- {{ __t("Số CMND/ Hộ chiếu") }}:</b
      ><span class="tab404">{{ Customer.IdentificationCard }}</span>
     </p>
     <p>
      <b>- {{ __t("Vào viện lúc") }}:</b
      ><span class="tab404">{{ Customer.AdmittedDate }}</span>
     </p>
     <p>
      <b>- {{ __t("Thời gian khám thai gần đây nhất") }}:</b
      ><span class="tab404"
    ...
    <div class="row mt-20">
     <div class="col-md-6 col-sm-6 text-center">
      <template v-if="DataSubmit.Confirm && DataSubmit.Confirm.HospitalLeadership.ConfirmAt">
       <p class="text-center">
        {{
         DataSubmit.Confirm.HospitalLeadership.ConfirmAt
          | formatDateHourMinutesDDMMYYY
     ...
     <div class="col-md-6 col-sm-6 text-center">
      <template
       v-if="DataSubmit.Confirm.HeadOfDepartmentOrLeaderOfOnDutyTeam && DataSubmit.Confirm.HeadOfDepartmentOrLeaderOfOnDutyTeam.ConfirmAt"
      >
       <p class="text-center">
        {{
         DataSubmit.Confirm.HeadOfDepartmentOrLeaderOfOnDutyTeam.ConfirmAt
...
import CoordinatingWithThePatientService from '@/services/IPD/CoronaryIntervention/CoordinatingWithThePatientOPD'
...
export default {
  ...
  methods: {
    isConfirmed: function () {
      return (this.DataSubmit.Confirm.HeadOfDepartmentOrLeaderOfOnDutyTeam.ConfirmAt || this.DataSubmit.Confirm.HospitalLeadership.ConfirmAt || this.viewOnly || this.DataSubmit.IsLocked || (this.condition && this.condition === 'Nurse'))
    },
    print () {
    ...
    getDataSetup () {
      new CustomersOPD().find(this.$route.params.Id).then(response => {
    ...
    getData () {
      ...
      new CoordinatingWithThePatientService().find('A01_152_100122_VE/' + this._VisitId + '/' + this._ItemId).then(res => {
        ...
        if (this.DataSubmit.IsLocked || this.$route.name !== 'OPDCoordinatingWithThePatientItem' || this.$store.state.account.Position.includes('Nurse') || res.Confirm.HospitalLeadership.ConfirmAt || res.Confirm.HeadOfDepartmentOrLeaderOfOnDutyTeam.ConfirmAt) {
          this.readonly = true
        }
        this.loaded = true
        // this.isConfirmed()
        ...
        console.log('other', this.textOther1)
