<template>
  <div id="StandingOrder-Page" class="main-content">
    <h1 class="text-center">{{__t('general.ghi_nhan_thuc_hien_thuoc')}}</h1>
      ...
      <template :data="item" v-for="(item,index) in dataSubmit.Datas">
        ...
        <template v-else>
          <tr v-bind:key="index + 'ey'">
            ...
            <td class="wrap">{{item.Route}}</td>
            <td>
              <textarea-autosize v-if="!item.Id"  rows="2" class="form-control" :placeholder="__t('Nhập ghi chú')" v-model="item.Note"/>
          ...
          <tr v-bind:key="index + 'ez'">
            <td style="border: none;">
              <button v-if="!item.IsConfirm && item.Id" class="btn pull-left v-yellow-btn" type="button" @click="confirm(item)">{{__t('Bác sĩ xác nhận')}}</button>
            </td>
            <td  colspan="4" style="border: none;">
              <span v-if="item.MedicalStaffName" style="display: inline-flex;">
                <div><i class="v-icon v-icon-nurse"></i></div>
  ...
  methods: {
    ...
    removeData (index, item) {
      this.$modal.show('dialog', {
        title: 'Cảnh báo!',
        text: 'Bạn có muốn xóa lượt ghi nhận thực hiện thuốc này?',
