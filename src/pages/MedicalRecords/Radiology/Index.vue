<template>
  <div class="disable_ccp">
    <!-- <DropList @dropListData="handleData" :ItemLists="Datas"/> -->
    <div class="box-x-ray">
      <div class="mrb10 mt-10 flex on-top space-between" style="align-items: flex-start;">
        <div>
          <div class="fw600 mrr5 mrb10 white-space inline-block">{{__t('Từ ngày ')}}:</div>
          <VDateTimePicker5 class="mrr10 mrb10" style="display: inline-block;" :max="getMax()" v-model="parameters.FromDate" :format="vDateTimeFormat" :placeholder="__t('Từ ngày')"/>
          <div class="fw600 mrr5 mrb10 white-space inline-block">{{__t('Đến ngày ')}}:</div>
          ...
          <div class="fw600 mrr5 mrb10 white-space inline-block" v-if="!$route.params.VisitCode">{{__t('Lượt tiếp nhận')}}:</div>
          <VNumberInput class="mrr10 mrb10" style="display: inline-block;width: 100px;" v-if="!$route.params.VisitCode" maxlength="10" v-model="parameters.VisitCode" :placeholder="__t('Nhập')"/>
          <div class="fw600 mrr5 mrb10 white-space inline-block">{{__t('Nhóm dịch vụ')}}:</div>
      ...
      <table class="table table-striped table-bordered v-table" id="customer-tbl">
        <thead class="bg-1375ba">
          <tr>
            ...
            <th class="text-center vertical-align-middle" style="width: 130px;">{{__t('Ngày chỉ định')}}</th>
            <th class="text-center vertical-align-middle">{{__t('Người trả kết quả')}}</th>
            <th class="text-center vertical-align-middle" style="width: 130px;">{{__t('Ngày trả kết quả')}}</th>
        ...
        <tbody>
          ...
          <tr :key="index">
            ...
            <td class="text-center">
              <div v-if="item.ReportId && item.ResultBy && hasRole('VIEWDETAILTRADIOLOGY')" class="btn v-yellow-btn" type="button" @click="showPopupDetail(item.ReportId, item.ResultBy)">{{__t('Xem')}}</div>
              <div v-else class="no-wrap">{{__t('Chưa có kết quả')}}</div>
            </td>
          </tr>
          </template>
        </tbody>
    ...
    <PopupRadiology v-if="openPopup" @closePopup="openPopup = false" :ReportId="ReportId" :ResultBy="ResultBy"/>
  </div>
</template>
...
import PopupRadiology from '../popup/PopupRadiology.vue'
...
export default {
  ...
  props: ['PID'],
  components: {
    VDateTimePicker5, VSelect, PopupRadiology
  },
  data () {
    return {
      Datas: null,
  ...
  methods: {
    getData () {
      ...
      if (this.PID) {
        this.parameters.Assessor = this.PID
      }
      if (this.$route.params.VisitCode) {
      ...
      new MedicalRecordCDHA(this.parameters).find('').then(response => {
        this.Datas = response
    ...
    handleFilter () {
      ...
      if (this.parameters.CategoryCode) {
        result.map(item => {
          if (item.CategoryCode && item.CategoryCode.includes(this.parameters.CategoryCode)) {
            result1.push(item)
          }
        })
      } else {
      ...
      // this.ResultEmit = uniqBy(result3, e => e.ReportId)
    ...
    async showPopupDetail (ReportId, ResultBy) {
      this.ReportId = await ReportId
      this.ResultBy = await ResultBy
      this.openPopup = await true
      setTimeout(() => {
        this.$modal.show('PopupRadiology')
      }, 500)
    }
  }
}
