...
import PhysicianNoteEd from './PhysicianNote.vue'
...
export default {
  ...
  components: {
    Logs, VSelect, VDatePicker, DatePicker, Printer, PhysicianNoteEd, SingleConfirm
  },
  mounted () {
    this.checkview()
    this.getData()
  ...
  methods: {
    ...
    checkShowDetail (item) {
      let check = false
      if (!this.ViewOnly && !item.IsLocked && (this.IsUpdate === item.CreatedBy || this.ImSuperAdmin()) && (!item.ConfirmCreated || (item.ConfirmCreated && !item.ConfirmCreated.ConfirmAt))) {
        check = true
      }
      return check
    },
