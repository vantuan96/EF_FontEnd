          ...
          <tr>
            <td colspan="6">
              <form class="form-inline">
                ...
                <div class="form-group">
                  <v-date-picker v-model="parameters.StartAdmittedDate" :format="vDateTimeFormat"/>
                </div>
                <div class="form-group">
                  <v-date-picker v-model="parameters.EndAdmittedDate" :format="vDateTimeFormat"/>
                </div>
        ...
        <tbody>
          <template v-if="ED.length">
            ...
            <tr :data="item" :key="((parameters.pageNumber - 1) * parameters.PageSize) + (index + 1)" v-for="(item ,index) in ED">
              ...
              <td width="40">{{item.HandOverUnit}}</td>
...
import OPDService from '@/services/ED/HandOverCheckList'
...
export default {
  ...
  methods: {
    ...
    confirm () {
      ...
      new OPDHandoverService(`api/ED/HandOverCheckList/Accept/`).update(this.Item.Id, this.usr).then(response => {
