<template>
  <div class="main-content" id="StandingOrderForRetailService-page">
    <h2 class="text-center"><b>{{__t('Biên bản hội chẩn bệnh nhân sử dụng thuốc có dấu sao (*)')}}</b></h2>
    <h4 class="text-center"><b>{{__t('Khoa')}}: </b>{{this.$store.state.account.Specialty.ViName}}</h4>
    <div v-if="loaded">
        ...
        <div class="row">
          <div class="col-md-6 col-sm-6 text-center">
            ...
            <template v-else>
              ...
              <p><button @click="showDoctorConfirm('HospitalDirectorOrHeadDepartment', 'Giám đốc bệnh viện/Trưởng khoa')" class="btn v-white-btn">{{__t('Xác nhận')}}</button></p>
            </template>
          </div>
          <div class="col-md-6 col-sm-6 text-center">
            ...
            <template v-else>
              ...
              <p><button @click="showDoctorConfirm('Doctor', 'Bác sĩ điều trị')" class="btn v-white-btn">{{__t('Xác nhận')}}</button></p>
            </template>
          </div>
        </div>
        <br/><br/>
        <br/><br/>
        <logs :EdId="this.$route.params.Id" :Type="'ED/ConsultationDrugWithAnAsteriskMark'" />
...
export default {
  ...
  methods: {
    ...
    submit () {
      ...
      setTimeout(() => {
        new ConsultationDrugWithAnAsteriskMark().update(this.$route.params.Id, DataSubmit).then(response => {
          this.getData()
    ...
    confirm () {
      new ConsultationDrugWithAnAsteriskMark().update(this.$route.params.Id, this.user, '/Accept/').then(response => {
        this.getData()
        this.toastedSuccess().scroll2Bottom()
    ...
    creat () {
      new ConsultationDrugWithAnAsteriskMark().update(this.$route.params.Id, {}, '/Create/').then(response => {
        this.reload()
      }).catch(e => {
        this.back()
      })
    ...
    getData () {
      this.loaded = false
      new ConsultationDrugWithAnAsteriskMark().find(this.$route.params.Id + '?hidemsg=' + true).then(response => {
        if (response.Orders) {
        ...
        this.DataSubmit.confirmed = (this.DataSubmit.HospitalDirectorOrHeadDepartment.Username || this.DataSubmit.Doctor.Username)
    ...
    confirmPopup () {
      this.$modal.show('dialog', {
        clickToClose: false,
        title: 'Thông báo',
        text: 'Tạo mới biên bản hội chẩn thuốc có dấu sao(*)?',
