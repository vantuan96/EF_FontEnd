<template>
  <div class="main-content SummaryOf15DayTreatment disable_ccp5" id="SummaryOf15DayTreatment-page">
    <template v-if="loaded">
      <h1 class="text-center form-title">{{__t('Bảng sàng lọc nguy cơ tự hại')}}</h1>
      <br>
        ...
        <template>
          <ul class="nav nav-tabs flex-containerx no-border" role="tablist">
            <li :data="item" :key="index" v-for="(item ,index) in List" class="text-center flex-containerx no-border" :class="activeClass(item.Id)">
              <router-link class="no-border" :to="{name:'IPDSelfHarmRiskScreeningToolEDItem', params: { Id: $route.params.Id, Item: item.Id }}">
...
export default {
  ...
  methods: {
    getData () {
      this.loaded = false
      new SelfHarmRiskScreeningToolService().find('/All/' + this.$route.params.Id + '?hidemsg=' + true).then(response => {
        this.List = response.Datas
        this.IsLocked = response.IsLocked
        console.log('data', this.List)
        if (response.Datas.length > 0 && !this.$route.params.Item) {
          this.$router.push({name: 'IPDSelfHarmRiskScreeningToolEDItem', params: { Id: this.$route.params.Id, Item: response.Datas[response.Datas.length - 1].Id }})
        }
        this.loaded = true
      }).catch(() => {
        this.loaded = true
    ...
    handleCreate () {
      new SelfHarmRiskScreeningToolService().update('/Create/' + this.$route.params.Id).then(res => {
        this.toastedSuccess()
        this.$router.push({name: 'IPDSelfHarmRiskScreeningToolEDItem', params: { Id: this.$route.params.Id, Item: res.Id }})
      })
    },
    confirmPopup () {
      this.$modal.show('dialog', {
        ...
        text: this.__t('Tạo mới bảng sàng lọc nguy cơ tự hại ?'),
