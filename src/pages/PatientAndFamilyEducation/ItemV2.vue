<template>
  <div>
    <div v-if="!loading">
          ...
          <div class="group-radio">
            ...
            <template :data="item"  v-for="(item,index) in MASTERDATA['PFEFNPTBA'].Items">
              ...
              <span :key="index" v-else style="flex-grow: 1;">
                <textarea-autosize :code="MASTERDATA['PFEFNPTBA'].Items[2].Code" :resized="resized" :placeholder="__t('Ghi cụ thể')" class="form-control" v-model="item.Value" v-if="MASTERDATA['PFEFNPTBA'].Items[1].Value"/>
              </span>
            </template>
          </div>
        </div>
          ...
          <div class="col-md-12">
            ...
            <div class="group-checkbox">
              <template :data="item"  v-for="(item,index) in MASTERDATA['PFEFEL0'].Items">
                ...
                <span :key="index" v-else style="flex-grow: 1;">
                  <textarea-autosize :placeholder="__t('Ghi cụ thể')" :code="MASTERDATA['PFEFEL0'].Items[5].Code" class="form-control" v-model="item.Value" v-if="MASTERDATA['PFEFEL0'].Items[4] && MASTERDATA['PFEFEL0'].Items[4].Value"/>
                </span>
              </template>
            </div>
          </div>
            ...
            <div class="group-checkbox">
              ...
              <div :key="index" :data="item"  v-for="(item,index) in MASTERDATA['PFEFSPCH'].Items.filter(item => item.Code === 'PFEFSPCHSLB')">
                ...
                <span v-if="MASTERDATA['PFEFSPCH'].Items[1].Value">
                  <textarea-autosize :code="MASTERDATA['PFEFSPCHTEXT'].Items[0].Code" :placeholder="__t('Mô tả cụ thể (khi xác định có rào cản khiến người được GDSK không hiểu)')" class="form-control mb-10" v-model="MASTERDATA['PFEFSPCHTEXT'].Items[0].Value"/>
                </span>
              </div>
              ...
              <div :key="index" :data="item" v-for="(item,index) in MASTERDATA['PFEFSPCH'].Items.filter(item => item.Code === 'PFEFSPCHEMB')">
                ...
                <span v-if="MASTERDATA['PFEFSPCH'].Items[2].Value">
                  <textarea-autosize :code="MASTERDATA['PFEFSPCHTEXT'].Items[1].Code" :placeholder="__t('Mô tả cụ thể (khi xác định có rào cản khiến người được GDSK không hiểu)')" class="form-control mb-10" v-model="MASTERDATA['PFEFSPCHTEXT'].Items[1].Value"/>
                </span>
              </div>
              <!-- <span>
                Mô tả:
                    ...
                    <div class="tab-content">
                      ...
                      <p v-if="(Version >= 10) && it.CreatedBy" class="text-right">
                        <Confirm
                          ...
                          :showCopy="true"
                          :ReadOnly="true"
                          :AdInfo="[it.ConfirmCreated && it.ConfirmCreated.ConfirmAt ? it.ConfirmCreated.ConfirmBy : '']"
                          :VisitId="_VisitId"
                          :FormId="it.Id"
                          />
                        </p>
      ...
      <div class="status-float-block" v-if="!viewOnly" :openBack="true" @handleBack="handleBack">
...
export default {
  ...
  methods: {
    ...
    clearCheckBox (index) {
      ...
      if (index === 0) {
        this.MASTERDATA['PFEFSPCH'].Items[1].Value = false
      }
      if (index === 1) {
        this.MASTERDATA['PFEFSPCH'].Items[0].Value = false
      }
      if (index === 1) {
    ...
    validateForm () {
      ...
      var error = []
      // error.push(this.MASTERDATA['PFEFEN0PM0-PFEFTOE-2'].Items[2])
      if (this.MASTERDATA['PFEFNPTBA'].Items[1].Value && !this.MASTERDATA['PFEFNPTBA'].Items[2].Value) {
        error.push(this.MASTERDATA['PFEFNPTBA'].Items[2])
      }
      if (this.MASTERDATA['PFEFEL0'].Items[4].Value && !this.MASTERDATA['PFEFEL0'].Items[5].Value) {
        error.push(this.MASTERDATA['PFEFEL0'].Items[5])
      }
      if (this.MASTERDATA['PFEFSPCH'].Items[1].Value && !this.MASTERDATA['PFEFSPCHTEXT'].Items[0].Value) {
        error.push(this.MASTERDATA['PFEFSPCHTEXT'].Items[0])
      }
      if (this.MASTERDATA['PFEFSPCH'].Items[2].Value && !this.MASTERDATA['PFEFSPCHTEXT'].Items[1].Value) {
        error.push(this.MASTERDATA['PFEFSPCHTEXT'].Items[1])
      }
    ...
    validateItem () {
      this.countErr = 0
      for (let i = 0; i < this.EducationalNeeds.length; i++) {
        for (let j = 0; j < this.EducationalNeeds[[i]].Datas.length; j++) {
          for (let k = 0; k < this.EducationalNeeds[i].Datas[j].Datas.length; k++) {
            if (this.EducationalNeeds[i].Datas[j].Datas[k].Code === 'PFEFTOEOTH') {
              const itemCheck = this.EducationalNeeds[i].Datas[j].Datas.filter(item => item.Code === 'PFEFTOEANS')
              console.log('itc', this.EducationalNeeds[i].Datas[j].Datas, itemCheck[0])
              if (itemCheck && !itemCheck[0].Value) {
      ...
      console.log('this.countErr', this.countErr)
    },
    mappingData (confirm) {
      // PFEFTOE
      console.log('EducationalNeeds ', this.EducationalNeeds)
      this.validateForm()
      console.log('err', this.countErr)
      NProgress.start()
      // this.Informations = response.Informations
      // this.GroupContents = response.GroupContents
      var submitData = {
