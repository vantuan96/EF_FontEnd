<template>
  <div class="page-table-vital">
    <!-- page chart -->
    <page-chart v-if="true" :FormType="'IPDNeonatalVitalSign'" :viewOnly="viewOnly" :parameters="parameters" :DataChart="DataChart" :VisitId="VisitId"/>
    <!-- box date time -->
    ...
    <div class="box-table mrb30">
      <table-form v-if="false" @editId="handleEdit" :FormType="'IPDNeonatalVitalSign'" :viewOnly="false" :parameters="parameters" :DataChart="DataChart" :VisitId="VisitId"/>
      <table v-else class="standing-order-tbl">
        <tr>
          ...
          <th width="50px" class="text-center">{{__t('Tổng NEWS')}}</th>
        ...
        <template v-else v-for="(item, index) in data">
          <tr :key="index + '-root-tr'" :class="{'view-full': item.view}">
            ...
            <td style="vertical-align:top" class="text-center" :class="getColor(item.MEWSTotal, item.IsYellow)">{{item.MEWSTotal || 'N/A'}}</td>
...
import PageChart from '@/components/IPD/VitalSigns/NeonatalChart.vue'
import TableForm from '@/components/IPD/VitalSigns/NeonatalTable.vue'
...
export default {
  ...
  mounted () {
    ...
    EventBus.$on('reloadUpdateFormNeonatal', this.handleLoc)
    EventBus.$on('reloadTableNeonatalVitalSigns', this.getData)
  },
  beforeDestroy () {
    EventBus.$off('reloadUpdateFormNeonatal')
    EventBus.$off('reloadTableNeonatalVitalSigns')
  },
  methods: {
    getData () {
      console.log('PageTable ~ getData')
      ...
      //   this.parameters.Assessor = ''
      // }
      new IPDNeonatalVitalSign(this.parameters).find('GetByVisitId/' + this._VisitId).then(response => {
        this.DataChart = response
        this.totalRow = response.Count
        this.IsLocked = response.Is24hLocked
        if (response.Is24hLocked) {
        ...
        var bonus = 1
        if ((this.totalRow % this.parameters.PageSize) === 0) {
          bonus = 0
        }
        this.parameters.pages = parseInt(this.totalRow / this.parameters.PageSize) + bonus
    ...
    handleLoc () {
      ...
      this.getData()
      EventBus.$emit('reloadChart', this.parameters)
    },
    handleXoaLoc () {
