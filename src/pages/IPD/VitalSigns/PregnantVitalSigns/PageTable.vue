<template>
  <div class="page-table-vital">
    <!-- page chart -->
    <page-chart v-if="true" :viewOnly="viewOnly" :parameters="parameters" :DataChart="DataChart" :VisitId="VisitId"/>
    <!-- box date time -->
    ...
    <div class="box-table mrb30">
      <table-form v-if="false" @editId="handleEdit" :VisitId="VisitId"/>
...
import TableForm from '@/components/IPD/VitalSigns/PregnantTable.vue'
...
export default {
  ...
  mounted () {
    this.getData()
    EventBus.$on('reloadUpdateFormPregnant', this.handleLoc)
    EventBus.$on('reloadTablePregnantVitalSigns', this.getData)
  },
  beforeDestroy () {
    EventBus.$off('reloadPregnantChart')
    EventBus.$off('reloadUpdateFormPregnant')
  ...
  methods: {
    getData () {
      ...
      //   this.parameters.Assessor = ''
      // }
      new IPDPregnantVitalSign(this.parameters).find('GetByVisitId/' + this._VisitId).then(response => {
    ...
    handleLoc () {
      ...
      this.getData()
      EventBus.$emit('reloadPregnantChart', this.parameters)
    },
    handleXoaLoc () {
