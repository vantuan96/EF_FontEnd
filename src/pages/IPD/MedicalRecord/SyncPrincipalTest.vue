<template>
  <div>
    ...
    <p class="text-right"><template v-if="syncAt"><i aria-hidden="true" class="fa fa-check v-green"></i> {{__t('Đồng bộ lần cuối lúc')}}: {{syncAt | formatDateTime('HH:mm:ss DD/MM/YYYY')}}</template><template v-else>{{__t('Đang tải...')}}</template></p>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    SyncPrincipalTest () {
      ...
      new MedicalRecord({hidemsg: true, noLoading: true}).update('Part3/SyncPrincipalTest/' + this.typeForm, {Id: this._VisitId}).then(response => {
