<template>
  <Vcollapse @getData="getData()" ref="Vcollapse" class="disable_ccp4">
    ...
    <template slot='info'>
      <span class="pull-right" style="padding: 8px;font-size: 14px;">{{__t('Chỉnh sửa lần cuối bởi')}} <AdInfo :ad="FormV2 ? FormV2.UpdatedBy : Form.UpdatedBy" /> {{__t('lúc')}} {{pkFormat ? $options.filters.formatDateWithoutSecon(Form.UpdatedAt) : FormV2 ? getFTime(FormV2.UpdateAt) : getFTime(Form.UpdatedAt)}}</span>
    </template>
    <template slot='content' v-if="hasData" class=" no-padding">
      <ViewOnly v-if="this.Form.Type === 'InitialAssessmentForAudult'" :MASTERDATA="MASTERDATA" :DataSubmit="DataSubmit" />
...
export default {
  name: 'InitialAssessmentForAudult',
  props: ['VisitId', 'Form', 'FormV2', 'pkFormat'],
  ...
  methods: {
    getData () {
      var formCode = this.Form.Type === 'InitialAssessmentForAudult' ? 'A02_013_220321_VE' : 'A02_014_220321_VE'
