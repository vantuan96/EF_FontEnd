<template>
  <div id="ACTForms-Page" class="main-content disable_ccp">
    <h2 class="text-center cap mrb30 fw600">{{__t('Xét nghiệm tại chỗ - ACT Cartridge Celite')}}</h2>
    <template v-if="loaded">
        ...
        <template>
          <ul class="nav nav-tabs flex-containerx no-border" role="tablist">
            <li :data="item" :key="index" v-for="(item ,index) in List" class="text-center flex-containerx">
              <router-link class="no-border" :to="{name:`${_VisitType}ACTFormsItem`, params: { Id: _VisitId, Item: item.Id }}">
      ...
      <div class="text-center" v-else>
        ...
        <!-- <h4>{{__t('Chưa có Xét nghiệm tại chỗ - ACT Cartridge Celite')}}</h4>
  ...
  methods: {
      ...
      this.APIService.GetList().then(res => {
        ...
        if (res.FORM_NOT_FOUND && res.FORM_NOT_FOUND.ViMessage === 'Form không tồn tại') {
          ...
          if (res.Datas && res.Datas.length > 0 && !this.$route.params.Item) {
            ...
            this.$router.push(
              {
                name: `${this._VisitType}ACTFormsItem`,
    ...
    confirmPopup () {
      this.$modal.show('dialog', {
        clickToClose: false,
        title: this.$t('Xét nghiệm tại chỗ - ACT Cartridge Celite'),
        text: this.$t('Tạo mới Xét nghiệm tại chỗ - ACT Cartridge Celite\nMáy: iSTAT'),
