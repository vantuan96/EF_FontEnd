    ...
    <div class="a4-page box" id="printMe">
      <div class="form-print">
        <template v-if="hasData">
          <table class="table" style="width: 100%;margin-bottom: 0px!important;">
            <tr>
              ...
              <td style="vertical-align: top;" align="top">
                <div v-if="DataSubmit && DataSubmit.Specialty">
                  <div class="cap" style="font-size: 16px;white-space: nowrap;">KHOA/ <i>Department</i>: {{DataSubmit.Specialty.ViName}}/ <i>{{DataSubmit.Specialty.EnName}}</i></div>
                </div>
              </td>
            </tr>
          </table>
          ...
          <div class="fs16 fw600 mrt10 text-center cap">Xét nghiệm tại chỗ/ <i>Point - of - care testing (poct)</i></div>
          <div class="fs16 fw600 mrt10 text-center">ACT (Catridge ACT Celite)</div>
          <div class="fs16 fw600 mrt10 text-center">Máy/ <i>Equipment</i>: iSTAT</div>
          <table class="tbl-b tbl-c mrt10" style="width: 100%">
            <tr>
              ...
              <td class="text-center content-center fw600" rowspan="2" width="90px" style="padding-top: 9px!important;">Chỉ số báo động/ <i>Critical Result</i></td>
            ...
            <tr style="height: 200px;">
              ...
              <td class="text-center">{{getValueOfMASTERDATA('HLQACT001', 'HLQACT002')}}</td>
              <td class="text-center">giây/<br/><i>sec</i></td>
          ...
          <div class="box-ngang mrt20">
            <div>
              <div v-if="Date1 && House1 && House1.length && Date1.length">Thời gian/ <i>Time</i>: {{House1[0] ? House1[0] : `&emsp;&emsp;`}}:{{House1[1] ? House1[1] : `&emsp;&emsp;`}}, ngày/ <i>date</i>: {{Date1[0] ? Date1[0] : `&emsp;&emsp;`}}/{{Date1[1] ? Date1[1] : `&emsp;&emsp;`}}/{{Date1[2] ? Date1[2] : `&emsp;&emsp;`}}</div>
          ...
          <div>Dán kết quả</div>
          <div v-if="this.MASTERDATA['HLQACT003'] && this.MASTERDATA['HLQACT003'].Items[0].Value">
            <template v-for="(src, index) in this.MASTERDATA['HLQACT003'].Items[0].Value">
              <img :key="index" :src="src" style="width: 100%;height: auto;"/>
            </template>
          </div>
        </template>
      </div>
    </div>
    <div class="a4-page code-form">A03_040_080322_V</div>
  </div>
</template>
...
export default {
  props: ['viewOnly', 'VisitId', 'VisitType', 'formId', 'Version'],
  name: 'act-print',
  ...
  computed: {
    ...
    Date1: function () {
      if (this.Technician && this.Technician.ConfirmAt && this.Version >= 7) {
        return this.getFTime(this.Technician.ConfirmAt).split(' ')[1].split('/') || ['', '', '']
      } else if (this.DataSubmit && this.Version < 7) {
    ...
    House1: function () {
      if (this.Technician && this.Technician.ConfirmAt && this.Version >= 7) {
        return this.getFTime(this.Technician.ConfirmAt).split(' ')[0].split(':') || ['', '']
      } else if (this.DataSubmit && this.Version < 7) {
        if (this.DataSubmit.UpdatedAt) {
  ...
  methods: {
    getData () {
      new API().find('GetDetail/' + this._VisitType + '/A03_040_080322_V/' + this._VisitId + '/' + this._ItemId + '?hidemsg=' + true).then(res => {
        ...
        if (res.data && res.data.ConfirmInfos && res.data.ConfirmInfos.length) {
          res.data.ConfirmInfos.find(e => {
            ...
            if (e.ConfirmType === 'Technician_UserCreated' && this.Version >= 7) {
              this.Technician = e
            }
          })
        }
