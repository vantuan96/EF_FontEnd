<template>
  <div id="CartridgeKaolinACT-Page" class="main-content disable_ccp">
        ...
        <template>
          <ul class="nav nav-tabs flex-containerx no-border" role="tablist">
            <li :data="item" :key="index" v-for="(item ,index) in List" class="text-center flex-containerx">
              <router-link class="no-border" :to="{name:`${_VisitType}CartridgeKaolinACTItem`, params: { Id: _VisitId, Item: item.Id }}">
  ...
  methods: {
      ...
      this.APIService.GetList().then(res => {
        ...
        if (res.FORM_NOT_FOUND && res.FORM_NOT_FOUND.ViMessage === 'Form không tồn tại') {
          ...
          if (res.Datas && res.Datas.length > 0 && !this.$route.params.Item) {
            ...
            this.$router.push(
              {
                name: `${this._VisitType}CartridgeKaolinACTItem`,
    ...
    confirmPopup () {
      this.$modal.show('dialog', {
        clickToClose: false,
        title: this.$t('Xét nghiệm tại chỗ - Đông máu ACT (Cartridge Kaolin ACT)'),
        text: this.$t('Tạo mới Xét nghiệm tại chỗ - Đông máu ACT (Cartridge Kaolin ACT)\nMáy: iSTAT'),
