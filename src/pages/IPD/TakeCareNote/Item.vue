      ...
      <div v-if="loaded">
        <template v-if="DataSubmit">
          <div class="form-content form-content-box">
            ...
            <modal name="EditNoteTake" transition="pop-out" height="auto" :clickToClose="false">
                ...
                <div class="box-body padding-15">
                  ...
                  <div class="form-group ct-form-group" style="display: flex">
                    <div class="inonelinebedroom">
                      ...
                      <textarea-autosize :maxHeight="100" v-model="editItem.Bed" class="form-control" rows="1"/>
                    </div>
                    <div class="inonelinebedroom">
                      <label>{{__t('Số buồng')}}</label>
                      <textarea-autosize :maxHeight="100" v-model="editItem.Room" class="form-control" rows="1"/>
                    </div>
                  </div>
                  <div class="form-group ct-form-group">
                    <label>{{__t('Theo dõi diễn biến')}}</label>
                    <textarea-autosize :maxHeight="300" v-model="editItem.ProgressNote" class="form-control" rows="3"/>
                  </div>
                  <div class="form-group ct-form-group">
                    <label>{{__t('Thực hiện y lệnh / chăm sóc')}}</label>
                    <textarea-autosize :maxHeight="300" v-model="editItem.CareNote" class="form-control" rows="3"/>
                  </div>
                </div>
          ...
          <FloatBlock v-if="!ViewOnly">
            <template slot='buttons'>
              ...
              <div class="col-md-4 col-sm-4 col-4" v-if="!DataSubmit.IsLocked && !isConfirmed">
                <template v-if="Version2">
                  <button v-if="!readonly" @click="openForm()" class="btn v-white-btn pull-right btn-block"><i aria-hidden="true" class="fa fa-plus"></i> {{__t('btn.them_cham_soc')}}</button>
                </template>
                <template v-else>
          ...
          <Printer :datasubmit="DataSubmit.Datas" :Data="DataSubmit" :FormIdNewborn="FormIdNewborn" :Customer="Customer" class="none"/>
        </template>
      </div>
      <div v-else class="loading-text"><v-loading/></div>
      <br/>
      <div>
        <!-- <logs v-if="!ViewOnly" :EdId="this.$route.params.Id" :Type="'IPD/CareNote'" :noDetail="true" /> -->
        <p class="mrt30">A02_062_050919_V</p>
...
import $ from 'jquery'
export default {
  name: 'TakeCareNoteIPDItem',
  props: ['VisitId', 'VisitType', 'ViewOnly', 'FormIdNewborn', 'Version2'],
  mixins: [MixinForm],
  data () {
    return {
      search: {
  ...
  computed: {
    ...
    readonly () {
      return this.ViewOnly || !this.hasRole('IPDPCC02') || (this.DataSubmit.DoctorTime || this.DataSubmit.HeadOfDepartmentTime || this.DataSubmit.HeadOfPharmacyTime || this.DataSubmit.IsLocked)
    }
  },
  methods: {
    checkview () {
    ...
    openForm (item) {
      if (item) {
        ...
        this.editItem = {
          ProgressNote: '',
          CareNote: '',
          Bed: this.DataSubmit.Datas[0] !== undefined ? this.DataSubmit.Datas[0].Bed : '',
          Room: this.DataSubmit.Datas[0] !== undefined ? this.DataSubmit.Datas[0].Room : ''
          // editItem.NoteTime || !editItem.ProgressNote || !editItem.CareNote || !editItem.Bed || !editItem.Room
        }
      }
      ...
      this.$modal.show('EditNoteTake')
    },
    close () {
      this.editItem = {}
      this.$modal.hide('EditNoteTake')
    },
    submit () {
    ...
    print () {
      this.$htmlToPaper('TakeCareNoteIPD', false, 'A02_062_050919_V')
    },
    async confirm () {
      if (this.edited) {
        await this.save()
    ...
    getData () {
      ...
      new TakeCareNote(this.ViewOnly ? {FormIdNewborn: this.FormIdNewborn} : {...this.search, FormIdNewborn: this.FormIdNewborn}, this._VisitType).find('list/' + this._VisitId).then(response => {
        this.DataSubmit = response
        this.Version = response.Version
        if (this.Customer && response.NewbornCustomer) {
          this.Customer = {
    ...
    updateInfo (cus) {
      new ContraintNewbornAndPregnantWoman().update('Updated/' + this.FormIdNewborn + '/' + this.Customer.PID).then(res => {
        this.toastedSuccess()
    ...
    readonly2 (item) {
      let check = false
      if (this.ViewOnly || !this.hasRole('IPDPCC02') || item.IsLocked || (item.ConfirmCreated && item.ConfirmCreated.ConfirmAt)) {
        check = true
      }
      return check
    }
