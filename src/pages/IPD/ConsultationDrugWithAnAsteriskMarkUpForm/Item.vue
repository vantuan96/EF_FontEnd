  ...
  <div class="main-content" id="StandingOrderForRetailService-page">
    <div v-if="loaded">
      <div v-if="DataSubmit" :class="{'r2eadonly': DataSubmit.confirmed}">
        ...
        <br/><br/>
        <br/><br/>
        <logs v-if="!viewOnly" :EdId="this.$route.params.Id" :Type="'ED/ConsultationDrugWithAnAsterisk'" />
...
export default {
  ...
  methods: {
    ...
    submit () {
      ...
      setTimeout(() => {
        if (this.$route.name === 'IPDConsultationDrugWithAnAsterisk' || this.$route.name === 'IPDDETAIL') {
          new ConsultationDrugWithAnAsteriskService().update('/' + this.$route.params.Id + '/' + this._ItemId, DataSubmit).then(response => {
          ...
          new ConsultationDrugWithAnAsteriskServiceED().update('/' + this.$route.params.Id + '/' + this._ItemId, DataSubmit).then(response => {
            if (this.isClickConfirm) {
              this.confirmSubmit()
            }
            this.getData()
    ...
    confirmSubmit () {
      if (this.$route.name === 'IPDConsultationDrugWithAnAsterisk' || this.$route.name === 'IPDDETAIL') {
        setTimeout(() => {
          new ConsultationDrugWithAnAsteriskService().update('/Accept/' + this.$route.params.Id + '/' + this._ItemId, this.user).then(response => {
            this.getData()
        ...
        setTimeout(() => {
          new ConsultationDrugWithAnAsteriskServiceED().update('/Accept/' + this.$route.params.Id + '/' + this._ItemId, this.user).then(response => {
            // this.getData()
            this.toastedSuccess(this.$t('Xác nhận thành công'))
            this.$modal.hide('doctorConfirm')
          }).catch(e => {
    ...
    creat () {
      new ConsultationDrugWithAnAsteriskService({}, this._VisitType).update(this.$route.params.Id, {}, '/Create/').then(response => {
        this.reload()
      }).catch(e => {
        this.reload()
      })
    ...
    getData () {
      this.loaded = false
      if (this.$route.name === 'IPDConsultationDrugWithAnAsterisk' || this.VisitType === 'IPD') {
        new ConsultationDrugWithAnAsteriskService().find(this._VisitId + '/' + this._ItemId + '?hidemsg=' + true).then(response => {
        ...
        new ConsultationDrugWithAnAsteriskServiceED().find(this._VisitId + '/' + this._ItemId + '?hidemsg=' + true).then(response => {
          this.IsLocked = response.IsLocked
          if (response.Orders) {
            response.Orders.map(e => {
              e.removed = false
