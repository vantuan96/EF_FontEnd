<template>
  <div class="main-content SummaryOf15DayTreatment disable_ccp5" id="SummaryOf15DayTreatment-page">
    <template v-if="loaded">
      <h1 class="text-center form-title">Điều trị can thiệp động mạch vành</h1>
      <br>
        ...
        <template>
          <ul class="nav nav-tabs flex-containerx no-border" role="tablist">
            <li :data="item" :key="index" v-for="(item ,index) in List" class="text-center flex-containerx no-border" :class="activeClass(item.Id)">
              <router-link class="no-border" :to="{name:'IPDCoronaryIntervention', params: { Id: $route.params.Id, Item: item.Id }}">
            ...
            <li class="text-center flex-container no-border" v-if="!IsLocked && condition==='Doctor'">
      ...
      <div class="text-center" v-else>
        ...
        <button class="btn v-yellow-btn" v-if="!IsLocked && condition==='Nurse'" @click="confirmPopup()">{{__t('Tạo mới')}}</button>
      </div>
    </template>
...
export default {
  ...
  methods: {
    getData () {
      this.loaded = false
      new CoronaryInterventionService().find(this.formCode + '/' + this.$route.params.Id + '?hidemsg=' + true).then(response => {
        this.List = response.Datas
        this.IsLocked = response.IsLocked
        console.log('data', this.Lisst)
        if (response.Datas.length > 0 && !this.$route.params.Item) {
          this.$router.push({name: 'IPDCoronaryIntervention', params: { Id: this.$route.params.Id, Item: response.Datas[response.Datas.length - 1].Id }})
        }
        this.loaded = true
      }).catch(() => {
        this.loaded = true
    ...
    handleCreate () {
      new CoronaryInterventionService().update('/Create/' + this.formCode + '/' + this.$route.params.Id).then(res => {
        this.toastedSuccess()
        this.$router.push({name: 'IPDCoronaryIntervention', params: { Id: this.$route.params.Id, Item: res.Id }})
      })
    },
    confirmPopup () {
      this.$modal.show('dialog', {
        ...
        title: 'Tạo phiếu điều trị can thiệp động mạch vành',
        text: 'Bạn có chắc chắn tạo phiếu điều trị can thiệp động mạch vành mới?',
