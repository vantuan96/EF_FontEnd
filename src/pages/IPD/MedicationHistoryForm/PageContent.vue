<template>
 <div class="MedicationHistoryForm-page">
   ...
   <div v-if="nullContent">
    <div v-if="IsLocked" style="color: red; margin-top: 30px;text-align: center;" class="italic text-center">
      <h4 class="text-center"><i class="fa fa-lock" aria-hidden="true"></i> {{__t('Bạn không được phép chỉnh sửa do hồ sơ đã bị khóa theo nguyên tắc 24h!')}}</h4>
    </div>
    <div v-else class="text-center" style="text-align: center;">
      <h4 class="italic text-center">{{message || __t('Chưa có thông tin')}}</h4>
    </div>
   ...
   <div class="content" v-else>
     ...
     <p class="fw600 fs16 block mrt10">{{__label(MASTERDATA['IPDMEDHIS05'])}}</p>
     <!-- check box -->
     <div class="flex-box flex-center" v-if="MASTERDATA['IPDMEDHIS05']">
        <span class="v-checkbox" align="center">
          <input type="checkbox" :disabled="isConfirmed ? true : false" :id="'IPDMEDHIS05-0'" :name="'IPDMEDHIS05-0'" @change="handleChange" v-model="MASTERDATA['IPDMEDHIS05'].Items[0].Value">
          <label :style="`cursor: ${isConfirmed ? 'context-menu' : null};`" :for="'IPDMEDHIS05-0'"></label>
        </span>
        <div style="width: 135px;">{{__label(MASTERDATA['IPDMEDHIS05'].Items[0])}}</div>
        <input :placeholder="__t('Ghi cụ thể2')" v-if="MASTERDATA['IPDMEDHIS05'].Items[0].Value" :readonly="isConfirmed" class="form-control" v-model="MASTERDATA['IPDMEDHIS05'].Items[1].Value" />
      </div>
     <br>
     <div class="flex-box flex-center" v-if="MASTERDATA['IPDMEDHIS05']">
        <span class="v-checkbox" align="center">
          <input type="checkbox" :disabled="isConfirmed ? true : false" :id="'IPDMEDHIS05-2'" :name="'IPDMEDHIS05-2'" @change="handleChange" v-model="MASTERDATA['IPDMEDHIS05'].Items[2].Value">
          <label :style="`cursor: ${isConfirmed ? 'context-menu' : null};`" :for="'IPDMEDHIS05-2'"></label>
        </span>
        <div style="width: 135px;">{{__label(MASTERDATA['IPDMEDHIS05'].Items[2])}}</div>
        <input :placeholder="__t('Ghi cụ thể2')" v-if="MASTERDATA['IPDMEDHIS05'].Items[2].Value" :readonly="isConfirmed" class="form-control" v-model="MASTERDATA['IPDMEDHIS05'].Items[3].Value" />
      </div>
     <hr class="mrt20">
     <p class="fw600 fs16 mrt10">{{__label(MASTERDATA['IPDMEDHIS10'])}} </p>
     <p class="fw600 fs16 block text-center mrt30">{{__label(MASTERDATA['IPDMEDHIS11'])}}</p>
     <MdCheckBoxLabel v-if="MASTERDATA['IPDMEDHIS11']" :readonly="isConfirmed" :data="MASTERDATA['IPDMEDHIS11']"/>
     <p class="fw600 fs16 block text-center mrt30">{{__label(MASTERDATA['IPDMEDHIS22'])}}</p>
     <MdCheckBoxLabel v-if="MASTERDATA['IPDMEDHIS22']" :readonly="isConfirmed" :data="MASTERDATA['IPDMEDHIS22']"/>
   ...
   <doctor-confirm :popupTitle="popupTitle" :TypeConfirm="TypeConfirm" @popupSave="popupSave"/>
   <p>A03_120_120421_VE</p>
...
// import $ from 'jquery'
export default {
  name: 'MedicationHistoryForm-page',
  ...
  computed: {
    getRoleCreate () {
      // if (this.$store.state.account.Position.includes('Doctor') || this.$store.state.account.Position.includes('Director')) {
      if (this.hasRole('MEDHIS1')) {
        return true
      } else {
        return false
      }
  ...
  methods: {
    ...
    handleCreateContent () {
      ...
      new GetMedicationHistory().update('/Create/A03_120_120421_VE/' + this._VisitId).then(() => {
        this.toastedSuccess()
        this.reload()
      }).catch(() => {
      })
    ...
    pushData () {
      if (!this._VisitId) return
      new GetMedicationHistory().find('/Info/A03_120_120421_VE/' + this._VisitId + '?hidemsg=' + true).then(res => {
        this.DataInfo = res
    ...
    getData () {
      this.loaded = false
      if (!this._VisitId) return
      new GetMedicationHistory().find('/A03_120_120421_VE/' + this._VisitId + '?hidemsg=' + true).then(res => {
    ...
    handleSubmit (data) {
      ...
      Data.MedicationHistory = data[0]
      Data.TPCN = data[1]
      new GetMedicationHistory().update('/Update/A03_120_120421_VE/' + this._VisitId, Data).then(() => {
        this.isedit = false
        if (data && data.length && data[2] === 'confirm') {
    ...
    async popupSave (data) {
      if (!this.isConfirmed && this.isedit) {
        await EventBus.$emit('SaveMedicationHistoryForm')
      }
    ...
    ConfirmKyNhay (data) {
      if (!this._VisitId) return
      setTimeout(() => {
        new GetMedicationHistory().update('/Confirm/A03_120_120421_VE/' + this._VisitId, data).then(() => {
          this.toastedSuccess()
          this.reload()
        }).catch(() => {
        })
